...
const DEFAULT_MENUS = ['head', 'bold', 'fontSize', 'fontName', 'italic', 'underline', 'strikeThrough', 'indent', 'lineHeight', 'foreColor', 'backColor', 'link', 'list', 'todo', 'justify', 'quote', 'emoticon', 'image', 'video', 'table', 'code', 'splitLine', 'undo', 'redo']
  ...
  methods: {
    replaceStringHtml(html) {
      ...
      if (html.indexOf('</span>') > -1) {
        return html.replace(/<span[^]*<\/span>/g, (str) => {
          return this.replaceSpan(str)
        })
      }
      return html
    },
    ...
    initEditor() {
      ...
      this.setMenus()
      this.bindEvent()
      pageEditorIns.customConfig.uploadImgServer = '/upload-img'
